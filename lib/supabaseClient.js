...
export async function addToSupabaseWatchlist(userId, item) {
  ...
  return safeSupabaseResult(
    supabase.from("user_watchlist").upsert(productItemRow(userId, item), { onConflict: "user_id,item_key" }).select("id,item_key").single(),
    "user_watchlist",
    "upsert",
    null
  );
...
export async function saveMovieGramRemoteState(userId, state) {
  ...
  if (episodeRows.length) {
    ...
    const existing = await assertSupabase(
      supabase
        .from("episode_progress")
    ...
    if (staleRows.length) {
      await Promise.all(staleRows.map((row) => assertSupabase(
        supabase
          .from("episode_progress")
          .delete()
          .eq("user_id", userId)
    ...
    await assertSupabase(supabase.from("episode_progress").delete().eq("user_id", userId), "episode_progress", "delete all");
  }
...
export async function createActivityEvent(userId, action, item, metadata = {}) {
  ...
  const { error } = await supabase.from("activity_events").upsert({
    ...
    poster_path: hasItem ? item.poster_path || null : null,
    item_data: hasItem ? item : {},
    metadata,
