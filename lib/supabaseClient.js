...
export function itemKey(item = {}) {
  ...
  return tmdbId ? `${type}:${tmdbId}` : `title:${(item.title || item.name || "untitled").toLowerCase()}`;
}
...
async function safeSupabaseStatus(resultPromise, table, action, fallback = null) {
  if (!supabase) return { data: fallback, ok: false, skipped: true };
  try {
    const result = await resultPromise;
    if (result.error) {
      warnRemoteSync(table, action, result.error);
      if (isRecoverableRemoteSyncError(result.error)) remoteSyncDisabledTables.add(table);
      return { data: fallback, ok: false, error: result.error };
    }
    return { data: result.data ?? fallback, ok: true };
  } catch (error) {
...
export async function loadProductLibrary(userId) {
  if (!supabase || !userId) return null;
  const [watchlistResult, watchedResult, reviewResult, listRows] = await Promise.all([
    safeSupabaseStatus(
      supabase.from("user_watchlist").select("item_key,tmdb_id,media_type,title,poster_path,release_year,created_at,watch_asap,watch_asap_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(1000),
      "user_watchlist",
      "load",
      []
    ),
    safeSupabaseStatus(
      supabase.from("user_watched").select("item_key,tmdb_id,media_type,title,poster_path,release_year,watched_at,created_at").eq("user_id", userId).order("watched_at", { ascending: false }).limit(1000),
      "user_watched",
      "load",
      []
    ),
    safeSupabaseStatus(
      supabase.from("ratings_reviews").select("item_key,tmdb_id,media_type,title,poster_path,release_year,rating,review_text,contains_spoiler,visibility,created_at,updated_at").eq("user_id", userId).order("updated_at", { ascending: false }).limit(1000),
      "ratings_reviews",
      "load",
      []
    ),
  ...
  const failed = [watchlistResult, watchedResult, reviewResult].some((result) => !result.ok && !result.skipped);
...
export async function loadRecentActivity(userId, limit = 30) {
  if (!supabase || !userId) return null;
  if (!acquireRemoteLoadLock("recentActivity", userId, 15000)) {
    return { __remoteLoadSkipped: true, __skipReason: "REMOTE_LOAD_LOCK" };
  }
  const result = await safeSupabaseStatus(
    supabase.from("activity_events").select("user_id,event_key,action,type,item_key,tmdb_id,media_type,title,poster_path,item_data,metadata,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(Math.min(Math.max(Number(limit) || 30, 1), 30)),
    "activity_events",
    "load recent",
    []
  );
  extendRemoteLoadCooldown("recentActivity", userId, 15000);
...
export async function addItemToList(userId, listId, item, sortOrder = 0) {
  ...
  const membershipRow = productItemRow(userId, item, {
    list_id: listId,
    sort_order: sortOrder
  });
  delete membershipRow.release_year;
  return safeSupabaseResult(
    supabase.from("user_list_items").upsert(membershipRow, { onConflict: "list_id,item_key" }).select("id,list_id,item_key").single(),
    "user_list_items",
    "upsert",
    null
  );
...
export async function removeItemFromList(userId, listId, item) {
  if (!supabase || !userId || !listId || !item) return null;
  const row = productItemRow(userId, item, { list_id: listId });
  return safeSupabaseResult(
    supabase.from("user_list_items").delete().eq("user_id", userId).eq("list_id", listId).eq("item_key", row.item_key).select("id"),
    "user_list_items",
    "delete",
    null
  );
...
export async function followUser(followerId, profile, status = null) {
  ...
  return safeSupabaseResult(
    supabase.rpc("request_follow", { p_target_user_id: profile.id }).maybeSingle(),
    "follows",
    "request",
    null
  );
...
export async function createNotification({ userId, actorId = null, type, entityType = null, entityId = null, message = "", metadata = {} } = {}) {
  ...
  return safeSupabaseResult(
    supabase.rpc("enqueue_notification", {
      ...
      p_message: message,
      p_metadata: metadata,
      p_dedupe_key: metadata?.dedupe_key || null,
      p_artwork_url: metadata?.artwork_url || null
    }),
    "notifications",
    "insert",
    null
...
export async function markNotificationRead(notificationId, metadataPatch = {}) {
  ...
  return safeSupabaseResult(
    supabase.rpc("mark_notifications_read", { p_notification_ids: [notificationId] }),
    "notifications",
    "mark read",
    null
  );
...
export async function acceptFollowRequest(currentUserId, requesterId) {
  ...
  return safeSupabaseResult(
    supabase.rpc("respond_to_follow_request", { p_requester_id: requesterId, p_accept: true }).maybeSingle(),
    "follows",
    "accept",
    null
  );
...
export async function declineFollowRequest(currentUserId, requesterId) {
  if (!supabase || !currentUserId || !requesterId) return null;
  return safeSupabaseResult(
    supabase.rpc("respond_to_follow_request", { p_requester_id: requesterId, p_accept: false }).maybeSingle(),
    "follows",
    "decline",
    null
  );
...
const REMOTE_LOAD_LOCK_TTL_MS = 25000;
const remoteTabId = Math.random().toString(36).slice(2);
const remoteLoadsInFlight = new Set();
const remoteLoadCooldowns = new Map();
...
function remoteLoadLockKey(scope, userId) {
  return `moviegram.remoteLoadLock:${scope}:${userId || "guest"}`;
}

function acquireRemoteLoadLock(scope, userId, ttlMs = REMOTE_LOAD_LOCK_TTL_MS) {
  if (typeof window === "undefined" || !userId) return true;
  const key = remoteLoadLockKey(scope, userId);
  const loadKey = `${scope}:${userId}`;
  const now = Date.now();
  if (remoteLoadsInFlight.has(loadKey) || (remoteLoadCooldowns.get(loadKey) || 0) > now) return false;
  try {
    const current = JSON.parse(window.localStorage.getItem(key) || "null");
    if (current?.expiresAt && current.expiresAt > now) return false;
    window.localStorage.setItem(key, JSON.stringify({ owner: remoteTabId, expiresAt: now + ttlMs }));
    remoteLoadsInFlight.add(loadKey);
    return true;
  } catch {
    remoteLoadsInFlight.add(loadKey);
    return true;
...
function extendRemoteLoadCooldown(scope, userId, ttlMs = REMOTE_LOAD_LOCK_TTL_MS) {
  if (typeof window === "undefined" || !userId) return;
  const loadKey = `${scope}:${userId}`;
  remoteLoadsInFlight.delete(loadKey);
  remoteLoadCooldowns.set(loadKey, Date.now() + ttlMs);
  try {
    window.localStorage.setItem(remoteLoadLockKey(scope, userId), JSON.stringify({ owner: remoteTabId, expiresAt: Date.now() + ttlMs }));
  } catch {
    // localStorage can be unavailable in private modes; remote safety still works without cross-tab throttle.
  }
}

export async function loadMovieGramRemoteState(userId) {
  if (!supabase || !userId) return null;
  if (!acquireRemoteLoadLock("library", userId)) {
    return { __remoteLoadSkipped: true, __skipReason: "REMOTE_LOAD_LOCK", __failed: false, __failedTables: [] };
  }
  async function loadTable(table, request) {
    if (remoteSyncDisabledTables.has(table)) return { data: [], error: null, skipped: true, failed: true };
    try {
      const result = await request;
      if (result.error) {
  ...
  (items.data || []).forEach((row) => {
    const item = {
      ...(row.item_data || {}),
      id: row.item_data?.id || row.tmdb_id,
      tmdb_id: row.tmdb_id,
      item_key: row.item_key || `${row.media_type}:${row.tmdb_id}`,
      media_type: row.media_type,
      updatedAt: row.updated_at || row.item_data?.updatedAt,
      updated_at: row.updated_at || row.item_data?.updated_at
    };
  ...
  const failedTables = [items, episodes, reviews, lists, listItems].map((result, index) => result.failed ? tableRequests[index][0] : "").filter(Boolean);
  extendRemoteLoadCooldown("library", userId);
  return { watchlist, watched, favorites, ratings, reviews: reviewMap, episodeProgress, customLists, __failed: failedTables.length > 0, __failedTables: failedTables };
}
...
export async function saveMovieGramRemoteState(userId, state) {
  ...
  Object.values(state.removedItems || {}).forEach((entry) => {
    if (entry?.item_key) keys.add(entry.item_key);
  });

  const itemRows = [...keys].map((key) => {
    const item = state.watched?.[key] || state.watchlist?.[key] || state.favorites?.[key] || state.reviews?.[key]?.item || {};
    const removedEntry = Object.values(state.removedItems || {}).find((entry) => entry?.item_key === key);
    ...
    return {
      ...
      is_watched: removedEntry?.type === "watched" ? false : Boolean(state.watched?.[key]),
      is_watchlisted: removedEntry?.type === "watchlist" ? false : Boolean(state.watchlist?.[key]) && !state.watched?.[key],
      is_favorite: Boolean(state.favorites?.[key]),
      user_rating: state.ratings?.[key] || null,
      watched_at: removedEntry?.type === "watched" ? null : state.watched?.[key]?.watchedAt || null,
      watched_date_unknown: removedEntry?.type === "watched" ? false : Boolean(state.watched?.[key] && !state.watched?.[key]?.watchedAt),
      liked_at: state.favorites?.[key]?.likedAt || null,
      updated_at: removedEntry?.removed_at || new Date().toISOString()
    };
  }).filter(Boolean);
...
export async function backfillActivityEventPoster(userId, item, { posterPath = "", backdropPath = "" } = {}) {
  ...
  const key = itemKey(item);
  const parts = stableItemParts(key, item);
  const updateByItemKey = await safeRemoteSync(
    supabase
      .from("activity_events")
      .update({ poster_path: posterPath })
      .eq("user_id", userId)
      .eq("item_key", parts.itemKey),
    "activity_events",
    "poster update"
  );
  if (updateByItemKey.ok || !parts.tmdbId) return { updated: updateByItemKey.ok ? 1 : 0 };
  const updateByTmdb = await safeRemoteSync(
    supabase
      .from("activity_events")
      .update({ poster_path: posterPath })
      .eq("user_id", userId)
      .eq("tmdb_id", parts.tmdbId)
      .eq("media_type", parts.mediaType),
    "activity_events",
    "poster update by tmdb"
  );
  return { updated: updateByTmdb.ok ? 1 : 0 };
}
