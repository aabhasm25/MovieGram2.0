...
function normalizeTitle(value = "") {
  return decodeHtmlEntities(value).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
}

function titleYear(item = {}) {
  const raw = item.year || item.release_year || item.release_date || item.first_air_date || "";
  const match = String(raw).match(/\b(19|20)\d{2}\b/);
  return match ? match[0] : "";
}

function sequelSafety(candidate = {}, item = {}, itemTitle = "", haystack = "") {
  if (!itemTitle) return { safe: true, reason: "" };
  const titleText = normalizeTitle(candidate.video_title || candidate.videoTitle || "");
  const itemYear = titleYear(item);
  const candidateYears = [...new Set((titleText.match(/\b(19|20)\d{2}\b/g) || []))];
  if (itemYear && candidateYears.length && !candidateYears.includes(itemYear)) {
    return { safe: false, reason: "wrong_sequel_or_year" };
  }
  const sequelTerms = [
    "resurrections",
    "reloaded",
  ...
  const itemHasTerm = (term) => itemTitle.includes(term);
  const candidateHasTerm = (term) => titleText.includes(`${itemTitle} ${term}`) || (titleText.includes(term) && titleText.split(" ").some((word) => itemTitle.includes(word) && word.length > 3));
  const wrongSubtitle = sequelTerms.find((term) => candidateHasTerm(term) && !itemHasTerm(term));
  if (wrongSubtitle) return { safe: false, reason: "wrong_sequel_or_year" };
  const franchiseBases = ["captain america", "spider man", "matrix", "the matrix"];
  const wrongFranchiseTitle = franchiseBases.some((base) => itemTitle.includes(base) && titleText.includes(base) && !titleText.includes(itemTitle));
  if (wrongFranchiseTitle) return { safe: false, reason: "wrong_sequel_or_year" };
...
export function candidateLooksRelevant(candidate = {}, item = {}) {
  return assessCandidateRelevance(candidate, item).accepted;
}

export function assessCandidateRelevance(candidate = {}, item = {}) {
  const itemTitle = normalizeTitle(item.title || item.name || candidate.title || "");
  const videoTitle = normalizeTitle(candidate.video_title || candidate.videoTitle || "");
  const candidateTitle = normalizeTitle(candidate.title || "");
  const channel = normalizeTitle(`${candidate.channel_title || ""} ${candidate.creator_username || ""}`);
  const url = normalizeTitle(`${candidate.source_url || ""} ${candidate.watch_url || ""}`);
  const haystack = `${candidateTitle} ${videoTitle} ${channel} ${url}`.trim();
  const keywords = /\b(movie|film|scene|clip|short|trailer|teaser|featurette|behind the scenes|official)\b/i;
  const spam = /\b(motivation|motivational|philosophy|mindset|system|escape the matrix|red pill|sigma|wealth|grindset|inspirational)\b/i;
  const hasThumbnail = Boolean(candidate.thumbnail_url || candidate.thumbnailUrl);
  const trustedSource = /^(tmdb|official)$/i.test(candidate.channel_title || "")
    || /official|movieclips|rottentomatoes|warner|marvel|netflix|disney|pixar|universal|sony|prime video|hbo|a24|kinocheck/i.test(`${candidate.channel_title || ""} ${candidate.creator_username || ""} ${candidate.discovered_by || ""}`);
  const exactPhrase = itemTitle && haystack.includes(itemTitle);
  const titleWords = itemTitle.split(" ").filter((word) => word.length > 2 && !["the", "and"].includes(word));
  const wordMatches = titleWords.filter((word) => haystack.includes(word)).length;
  const titleKeywordMatch = wordMatches >= Math.min(2, titleWords.length || 1) && keywords.test(`${candidate.video_title || ""} ${candidate.title || ""}`);
  const tmdbExact = candidate.discovered_by === "admin_tmdb" || candidate.discovered_by === "admin_enrich_tmdb" || channel.includes("tmdb");
  const sequelCheck = sequelSafety(candidate, item, itemTitle, haystack);
  let matchScore = 0;
  if (exactPhrase) matchScore += 70;
  if (titleKeywordMatch) matchScore += 30;
  if (trustedSource) matchScore += 26;
  if (tmdbExact) matchScore += 45;
  if (hasThumbnail) matchScore += 8;
  if (!hasThumbnail) matchScore -= 25;
  const titleText = `${candidate.video_title || ""} ${candidate.title || ""}`;
  const spammy = spam.test(`${titleText} ${candidate.channel_title || ""}`) && !tmdbExact && !trustedSource;
  if (spammy) matchScore -= 90;
  if (!sequelCheck.safe) matchScore -= 120;
  const exactTitleIsSpecific = titleWords.length > 1 || keywords.test(titleText);
  const accepted = sequelCheck.safe && Boolean(tmdbExact || trustedSource || (exactPhrase && exactTitleIsSpecific) || titleKeywordMatch) && matchScore >= 35;
  return {
    accepted,
    match_score: matchScore,
    rejection_reason: accepted ? "" : sequelCheck.reason || (spammy ? "spam_channel" : !hasThumbnail ? "no_thumbnail" : "weak_match")
  };
}

export function scoreReelCandidate(candidate = {}, item = {}) {
  const normalized = normalizeReelCandidate(candidate);
  const relevance = assessCandidateRelevance(normalized, item);
  const formatScores = {
    short: 110,
    reel: 108,
    clip: 96,
  ...
  let score = Number(normalized.quality_score || 0);
  if (!relevance.accepted) return Math.min(15, score + relevance.match_score - 80);
  score += relevance.match_score + (formatScores[normalized.content_format] || 20);
