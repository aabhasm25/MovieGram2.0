...
function sequelSafety(candidate = {}, item = {}, itemTitle = "", haystack = "") {
  ...
  const titleText = normalizeTitle(`${candidate.video_title || candidate.videoTitle || ""} ${candidate.title || ""}`);
  ...
  const candidateHasTerm = (term) => haystack.includes(`${itemTitle} ${term}`) || titleText.includes(`${itemTitle} ${term}`);
...
export function assessCandidateRelevance(candidate = {}, item = {}) {
  ...
  if (spam.test(`${titleText} ${candidate.channel_title || ""}`) && !tmdbExact && !trustedSource) matchScore -= 90;
  ...
  return {
    ...
    rejection_reason: accepted ? "" : sequelCheck.reason || "Low title/source relevance"
  };
}
